...
export async function POST() {
  ...
  const r = await genJSON<any>(`${DATA_ANALYST_SYSTEM}\n\nData: ${s.total} responses, ${s.avgScreenTime}hrs avg, ${s.aiUsageRate}% AI users, ${s.hobbyToSkillRate}% hobby-to-skill, top platform ${s.topPlatform}, top skill ${s.topSkill}.`, { provider: "openrouter", model: "meta-llama/llama-3.3-70b-instruct", maxTokens: 1200 });
